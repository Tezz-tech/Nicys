...
const STATUS_STYLES = {
  ...
  'Delivered':   'bg-[#C4A8F0]/15 text-[#C4A8F0] border-[#C4A8F0]/30',
}

function Row({ label, value }) {
  if (!value && value !== 0) return null
  return (
    <div className="flex gap-4 py-3 border-b border-[#C4A8F0]/8 last:border-0">
...
function Section({ title, children }) {
  return (
    <section className="bg-[#1B2A4A] border border-[#C4A8F0]/12 rounded-sm p-6">
      <h2 className="font-body text-xs font-semibold tracking-widest uppercase text-[#C4A8F0]/60 mb-5">{title}</h2>
      {children}
    </section>
  )
}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {order.tier === 'scribe' && (order.littleThings || order.coreMemory || order.unspoken) && (
            <Section title="Scribe Prompts">
              <div className="bg-[#0d1627]/50 rounded-sm p-4 space-y-5">
                {order.littleThings && (
                  <div>
                    <p className="font-body text-[10px] tracking-widest uppercase text-[#C4A8F0]/50 mb-1.5">The Little Things</p>
                    <p className="font-body text-sm text-[#FFF8F0]/70 leading-relaxed whitespace-pre-wrap">{order.littleThings}</p>
                  </div>
                )}
                {order.coreMemory && (
                  <div>
                    <p className="font-body text-[10px] tracking-widest uppercase text-[#C4A8F0]/50 mb-1.5">The Core Memory</p>
                    <p className="font-body text-sm text-[#FFF8F0]/70 leading-relaxed whitespace-pre-wrap">{order.coreMemory}</p>
                  </div>
                )}
                {order.unspoken && (
                  <div>
                    <p className="font-body text-[10px] tracking-widest uppercase text-[#C4A8F0]/50 mb-1.5">The Unspoken</p>
                ...
                {order.letterImages.map((url, i) => {
                  ...
                  return (
                    <a key={i} href={url} target="_blank" rel="noreferrer"
                      className="block rounded-sm overflow-hidden border border-[#C4A8F0]/20 hover:border-[#C4A8F0]/50 transition-colors group"
                    >
                      {isPdf ? (
                        <div className="flex flex-col items-center justify-center bg-[#0d1627] py-8 px-4 text-center">
                          <span className="text-3xl text-[#C4A8F0]/40 mb-2">▣</span>
                          <p className="font-body text-xs text-[#C4A8F0]/40 group-hover:text-[#C4A8F0] transition-colors">Doc {i + 1}</p>
                        </div>
                      ) : (
                        <div>
                          <img src={url} alt={`File ${i + 1}`} className="w-full h-28 object-cover bg-[#0d1627]" />
                          <p className="font-body text-xs text-[#C4A8F0]/40 text-center py-1.5 bg-[#0d1627]/70 group-hover:text-[#C4A8F0] transition-colors">
                            Image {i + 1}
                          </p>
                        </div>
                      )}
            ...
            <Section title="Payment Proof">
              ...
              >
                {/\.pdf/i.test(order.paymentProofUrl) ? (
                  <div className="flex items-center justify-center gap-3 bg-[#0d1627] py-8">
                    <span className="text-2xl text-[#C4A8F0]/50">▣</span>
                    <p className="font-body text-sm text-[#C4A8F0]/50 group-hover:text-[#C4A8F0] transition-colors">Open PDF Receipt</p>
                  </div>
                ) : (
                  <div>
                    <img src={order.paymentProofUrl} alt="Payment receipt" className="w-full max-h-64 object-contain bg-[#0d1627]" />
                    <p className="font-body text-xs text-[#C4A8F0]/40 text-center py-2 bg-[#0d1627]/70">Click to open full size</p>
                  </div>
                )}
              </a>
            </Section>
          ...
          <Section title="Update Status">
            <div className="space-y-3">
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-[#0d1627] border border-[#C4A8F0]/20 rounded-sm px-4 py-3 font-body text-sm text-[#FFF8F0]/80 placeholder-[#FFF8F0]/20 focus:outline-none focus:border-[#C4A8F0]/50 transition-colors resize-none"
              />
              {error && <p className="font-body text-xs text-[#E8A0B4]">{error}</p>}
              <button
                onClick={handleUpdate}
                disabled={saving || (newStatus === order.status && !note.trim())}
                className="w-full bg-[#C4A8F0] hover:bg-[#b490e8] disabled:opacity-40 disabled:pointer-events-none text-[#0D1B38] font-body font-semibold text-sm tracking-widest uppercase py-3 rounded-sm transition-colors"
              >
            ...
            <Section title="Status History">
              <div className="space-y-2">
                {[...order.statusHistory].reverse().map((entry, i) => (
                  <div key={i} className="flex gap-3 bg-[#0d1627]/50 rounded-sm px-4 py-3">
